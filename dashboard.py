...
def overview_data(data):
    ...
    df.columns = ['ZIPCODE', 'TOTAL HOUSES', 'PRICE', 'SQRT LIVING', 'PRICE/PÉS']
    ...
    for name, row in df.iterrows():
        folium.Marker([row['lat'], row['long']],
                      popup='Sold ${0} on: {1}. Features: {2} sqft, {3} bedrooms,'
                            '{4} bathrooms, year built: {5}'.format(row['price'], row['date'], row['sqft_living'],
...
def commercial(data):
    ...
    st.sidebar.title('commeicial Options')
...
def attributes_distribution(data):
    # ----------------------------------------------------------------------
    # distribuição dos imoveis por categorias
    ...
    fig = px.histogram(data, x='bedrooms', nbins=15)
    c1.plotly_chart(fig, use_container_width=True)

    # House per bathrooms
    c2.header('Houses per bathrooms')
    df = data[data['bedrooms'] < f_bathrooms]
    fig = px.histogram(data, x='bathrooms', nbins=15)
...
def buy(data):
    ...
    st.title('Decision Analise')
    df2 = df2[['id','zipcode','price','price_median','status']]
